// Facebook Event Extractor Bookmarklet
// Safely extracts event data from Facebook event pages you're viewing manually
// No automation - just data extraction helper

(function() {
    // Create modal for displaying extracted data
    const modal = document.createElement('div');
    modal.id = 'event-extractor-modal';
    modal.style.cssText = `
        position: fixed;
        top: 50%;
        left: 50%;
    ...
    // Function to extract event data from current Facebook event page
    function extractEventData() {
        ...
        try {
            ...
            const titleSelectors = [
                ...
                '.x1heor9g .x1qlqyl8', // FB dynamic classes (may change)
                'h1', // Fallback
            ];
            ...
            for (const selector of dateTimeSelectors) {
                ...
                for (const el of elements) {
                    ...
                    if (text && (text.includes('202') || text.includes('AM') || text.includes('PM'))) {
                        dateTimeText += text + ' ';
                    }
                }
            ...
            const dateMatch = dateTimeText.match(datePattern);
            const timeMatch = dateTimeText.match(timePattern);
            
            if (dateMatch) eventData.date = dateMatch[1];
            if (timeMatch) eventData.time = timeMatch[1];
            ...
            for (const selector of locationSelectors) {
                ...
                if (locationEl && locationEl.textContent.trim()) {
                    eventData.venue = locationEl.textContent.trim();
                    break;
                }
            }

            // Extract organizer/promoter
            const organizerSelectors = [
                '[data-testid="event-permalink-organizer"] a',
                '.event-organizer a',
                '.hosted-by a',
                'a[role="link"][href*="/events"]'
            ];
            ...
            for (const selector of organizerSelectors) {
                ...
                if (organizerEl && organizerEl.textContent.trim()) {
                    eventData.organizer = organizerEl.textContent.trim();
                    break;
                }
            }
            ...
            for (const selector of descSelectors) {
                ...
                if (descEl && descEl.textContent.trim()) {
                    eventData.description = descEl.textContent.trim().substring(0, 300);
                    break;
                }
            }
    ...
    // Function to detect genre from event data
    ...
    modal.innerHTML = `
        ...
        <div style="margin-bottom: 20px;">
            ...
            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                <div style="margin-bottom: 8px;"><strong>Title:</strong> ${eventData.title || 'Not found'}</div>
                <div style="margin-bottom: 8px;"><strong>Date:</strong> ${eventData.date || 'Not found'}</div>
                <div style="margin-bottom: 8px;"><strong>Time:</strong> ${eventData.time || 'Not found'}</div>
                <div style="margin-bottom: 8px;"><strong>Venue:</strong> ${eventData.venue || 'Not found'}</div>
                <div style="margin-bottom: 8px;"><strong>Organizer:</strong> ${eventData.organizer || 'Not found'}</div>
                ...
                <div style="margin-bottom: 8px;"><strong>URL:</strong> <a href="${eventData.url}" target="_blank" style="color: #8b4aff;">Event Link</a></div>
            </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea id="json-output" style="width: 100%; height: 120px; font-family: monospace; font-size: 11px; border: 1px solid #ddd; border-radius: 4px; padding: 8px;" readonly>${JSON.stringify({
                source: 'facebook_bookmarklet',
    ...
    const sendBtn = modal.querySelector('#send-to-system');
    const closeBtn = modal.querySelector('#close-modal');
    const jsonOutput = modal.querySelector('#json-output');
    ...
    sendBtn.addEventListener('click', async () => {
        try {
            ...
            const response = await fetch('http://localhost:3001/api/events/import-from-email', {
                method: 'POST',
                headers: { 
            ...
            setTimeout(() => {
                sendBtn.textContent = '📤 Send to System';
                sendBtn.style.background = '#28a745';
            }, 3000);
        } catch (error) {
            console.error('Error sending data:', error);
            sendBtn.textContent = '❌ CORS Error';
            sendBtn.style.background = '#dc3545';
            ...
            `;
            errorDiv.innerHTML = `
                <strong>⚠️ Browser Security Block:</strong><br>
                The browser blocked the request from Facebook to localhost. 
                Please <strong>copy the data</strong> and manually paste it into your admin system at:
                <br><a href="http://localhost:3001/admin/import-review" target="_blank" style="color: #8b4aff;">http://localhost:3001/admin/import-review</a>
            `;
